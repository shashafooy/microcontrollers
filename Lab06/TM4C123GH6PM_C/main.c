...
volatile unsigned int currSine, numVoltage;

/**************************
Average voltage and use it 
to calculate frequency for 
Timer 2
**************************/
void WTIMER1A_Handler(void){
	unsigned int load;
	...
	WTIMER2->TAILR = load;//80000000*(1/freq);
	WTIMER2->CTL = 0x1;
}

/***********************
Send next sinusoid value to I2C
************************/
...
/**********************
Send 12 bits of data to the I2C module.
Only send start bit if nothing has been
send yet to I2C.
***********************/
void I2C_12bit(unsigned short data){
	//DAC address is 0x62
