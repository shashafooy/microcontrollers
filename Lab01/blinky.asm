...
loop
	MOV32 R1, #0x02	;load value for turning on LED colore ____
...
delay1
	ADD R4, #1			; ___________________________
	...
	MOV32 R1, #0x04	;load value for turning on LED color _____
...
delay2
	ADD R4, #1			;______________________________
	CMP R4, R5			;check number of iterations
	BLE delay2			;continue if iterated less than 0xFFFFF + 1 times, otherwise repeat delay loop

	MOV32 R1, #0x08	;load value for turning on LED _____
	STR R1, [R0, #0x38]	;write the above value to GPIOF ODR
